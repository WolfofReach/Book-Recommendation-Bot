...
class PassButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(f"✅ {interaction.user.mention} has passed on recommending.",
                                                ephemeral=False)
...
class CloseButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if interaction.user.id != session.starter_id:
            await interaction.response.send_message("Only the person who started the session can close it!",
                                                    ephemeral=True)
            return
...
async def close_and_pick_winner(interaction: discord.Interaction, session):
    ...
    if not all_books:
        await interaction.response.send_message("❌ No books were recommended! Session closed with no winner.",
                                                ephemeral=False)
